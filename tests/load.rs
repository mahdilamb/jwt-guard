...
async fn run_load(
    ...
    for _ in 0..concurrency {
        ...
        handles.push(tokio::spawn(async move {
            ...
            let mut warmup = client.get(format!("http://{addr}/load-test"));
            if let Some(ref a) = auth {
                warmup = warmup.header("Authorization", a);
            }
            warmup = warmup.header("Accept", "application/json");
            if let Ok(resp) = warmup.send().await {
...
async fn load_valid_tokens_50_concurrent() {
    ...
    assert_eq!(
        result.failed, 0,
        "no requests should fail at transport level"
    );
...
async fn load_mixed_valid_and_invalid() {
    ...
    let valid_handle = tokio::spawn(async move { run_load(addr, Some(valid_auth), 25, 20).await });
    let invalid_handle =
        tokio::spawn(
            async move { run_load(addr, Some("Bearer garbage".to_string()), 25, 20).await },
        );
